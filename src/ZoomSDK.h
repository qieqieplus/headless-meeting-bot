...
public:
  ...
  ZOOMSDK::SDKError initialize(const std::string &sdkKey,
                               const std::string &sdkSecret,
                               const std::string &zoomHost = "https://zoom.us");
  ZOOMSDK::SDKError
  authenticate(std::function<void()> onAuthCallback = nullptr);
  ...
  ZOOMSDK::ISettingService *getSettingService() const {
    return m_settingService;
  }
  ZOOMSDK::INetworkConnectionHelper *getNetworkHelper() const {
