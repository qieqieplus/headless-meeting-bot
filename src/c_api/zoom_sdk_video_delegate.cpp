...
void ZoomSDKVideoRendererDelegate::onRawDataFrameReceived(
    YUVRawDataI420 *data) {
  if (!data)
    return;
  ...
  if (yBuffer && uBuffer && vBuffer && bufferLen > 0) {
    ...
    Impl::zoom_meeting_dispatch_video(m_meetingHandle, yBuffer, uBuffer,
                                      vBuffer, width, height, bufferLen,
                                      sourceId, timestamp);
  }
}

void ZoomSDKVideoRendererDelegate::onRawDataStatusChanged(
