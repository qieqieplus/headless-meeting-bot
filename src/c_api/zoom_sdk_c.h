...
#if !defined(ZOOM_SDK_C_API)
#if defined(_WIN32) || defined(__CYGWIN__)
#ifdef ZOOM_SDK_C_BUILD
#define ZOOM_SDK_C_API __declspec(dllexport)
#else
#define ZOOM_SDK_C_API __declspec(dllimport)
#endif
#else
#define ZOOM_SDK_C_API __attribute__((visibility("default")))
#endif
#endif
...
typedef enum { ZOOM_SDK_SUCCESS = 0, ZOOM_SDK_ERROR = -1 } ZoomSDKResult;
...
typedef void (*OnAudioDataReceivedCallback)(MeetingHandle meeting_handle,
 ...
 */
ZOOM_SDK_C_API ZoomSDKHandle zoom_sdk_create(const char *sdk_key,
 ...
 */
ZOOM_SDK_C_API void zoom_sdk_destroy(ZoomSDKHandle handle);
 ...
 * camera feeds
 */
ZOOM_SDK_C_API MeetingHandle zoom_meeting_create_and_join(
    ZoomSDKHandle sdk_handle, const char *meeting_id, const char *password,
    const char *display_name, const char *join_token, int enable_audio,
    int enable_video);
 ...
 */
ZOOM_SDK_C_API void zoom_meeting_destroy(MeetingHandle meeting_handle);

/**
 * Get the current meeting status
 * @param meeting_handle The meeting handle
 * @return ZoomMeetingStatus indicating the current status, or
 * ZOOM_MEETING_STATUS_UNKNOWN if handle is invalid
 */
ZOOM_SDK_C_API ZoomMeetingStatus
zoom_meeting_get_status(MeetingHandle meeting_handle);
 ...
 */
ZOOM_SDK_C_API ZoomSDKResult zoom_meeting_set_audio_callback(
...
typedef struct {
  ...
  int segment_seconds;    // e.g., 10 (HLS segment duration)
  ...
  // Audio parameters
  int audio_sample_rate;  // e.g., 48000 (default), 44100, 32000, etc.
  int audio_channels;     // e.g., 2 (default stereo), 1 (mono)
 ...
 */
ZOOM_SDK_C_API ZoomSDKResult zoom_meeting_set_hls_video_callback(
    MeetingHandle meeting_handle, OnHlsFileCallback callback,
    const ZoomHlsVideoConfig *config);

/** Request an IDR (keyframe) from the encoder for the meeting. */
ZOOM_SDK_C_API ZoomSDKResult
 ...
 */
ZOOM_SDK_C_API void zoom_sdk_run_loop();
 ...
 */
ZOOM_SDK_C_API void zoom_sdk_stop_loop();
