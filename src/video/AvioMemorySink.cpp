...
int64_t AvioMemorySink::seek(void* opaque, int64_t offset, int whence) {
    ...
    // Optionally grow buffer when seeking forward beyond end; writer will fill later
...
int AvioMemorySink::openFile(AVFormatContext* s, AVIOContext** pb, const char* url, int flags) {
    ...
    AVIOContext* avioCtx = avio_alloc_context(buffer, AVIO_BUFFER_SIZE, 1, fb.get(),
                                              nullptr, AvioMemorySink::writePacket, AvioMemorySink::seek);
    if (!avioCtx) {
        av_free(buffer);
