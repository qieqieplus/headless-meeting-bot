...
class AuthServiceEvent : public ZOOMSDK::IAuthServiceEvent {
 ...
 public:
   ...
   * @param pAccountInfo Account Info that is only valid when ret ==
   * LOGINRET_SUCCESS
   ...
   * when triggered, please re-login or generate a new zoom access token via
   * REST Api.
   */
  void onZoomIdentityExpired() override;

  /**
   * callback used when Zoom authentication identity will be expired in 10
   * minutes when triggered please re-auth
   */
