...
public:
  MeetingShareEvent(
      std::function<void(const ZOOMSDK::ZoomSDKSharingSourceInfo &)>
          onShareStart,
      std::function<void(const ZOOMSDK::ZoomSDKSharingSourceInfo &)>
          onShareEnd);
  ...
  void onShareContentNotification(
  ...
  void onShareSettingTypeChangedNotification(
      ZOOMSDK::ShareSettingType type) override;
  void onSharedVideoEnded() override;
  void onVideoFileSharePlayError(
      ZOOMSDK::ZoomSDKVideoFileSharePlayError error) override;
